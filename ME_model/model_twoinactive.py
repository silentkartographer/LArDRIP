...
#from losses import masked_mse_loss, masked_mae_loss, Huber_loss, masked_mse_lossp, masked_mse_lossp_learn, masked_bce_loss, masked_mse_manyfeats
from model import *
from utils import *
...
batch_size = 20 #150 * 3
lr=1e-3 #1e-6 # 0.001 was 1e-5?
num_epochs = 150*20 #150*10
print_every = 10
model_name = "muons_with_E_new"

# W&B API Key
os.environ["WANDB_API_KEY"] = "your_key_hash"

# Initialize W&B
if args.resume and args.wandb_run_id:
    wandb.init(project='your_project', entity='your_username', id=args.wandb_run_id, resume="must")
else:
...
input_coords_path = r'/path_to/input_coords_energies.npz'
input_feats_path = r'/path_to/input_feats_energies.npz'
target_coords_path = r'/path_to/target_coords_energies.npz'
target_feats_path = r'/path_to/target_feats_energies.npz'
#grid_masks_path = r'/not_used!'
...
drop_last = False
# Create DataLoaders
train_DL = DataLoader(train_dataset, batch_size=batch_size, collate_fn=custom_collate_fn, shuffle=True, drop_last=drop_last, num_workers=4) 
test_DL = DataLoader(test_dataset, batch_size=batch_size, collate_fn=custom_collate_fn, shuffle=False, drop_last=drop_last, num_workers=4)
val_DL = DataLoader(val_dataset, batch_size=batch_size, collate_fn=custom_collate_fn, shuffle=False, drop_last=drop_last, num_workers=4)

in_channels = 1
out_channels = 1

model = CompletionNet_energy2_5().to(device)
#model = CompletionNet_256().to(device)
optimizer = optim.Adam(model.parameters(), lr=lr, amsgrad=True)

# lambda function to change learing rate at epoch = 4

# Function to update learning rate based on an external variable (epoch)
...
def train(model, train_loader, optimizer, epoch):
    ...
    for batch_idx, (input_coords, input_feats, target_coords, target_feats, masks, target_l) in enumerate(train_loader):
        ...
        out_tensor_with_E, targets, output_tensor_cls, losst, loss_cls, loss_energy = model(input_tensor, target_key, target_tensor, val_train=False, val_test=False, test=False)
        print(len(input_tensor), "->", len(out_tensor_with_E), "/", len(target_coords), 'target_coords_list: ', len(target_l))
        #target_tensor = ME.SparseTensor(features=target_feats, coordinates=target_coords, device=device)
        #resolution = get_dense_shape(target_tensor)
        #print(resolution)
        #dice = get_dc(model, resolution, input_tensor, output_tensor, target_coords, target_feats)
        #DC.append(dice)
        ...
        print(f"Train: Iter: {batch_idx}, Loss: {losst.item():.3f}, Loss_cls: {loss_cls.item():.3f}, Loss_energy: {loss_energy.item():.3f}")
        ...
        del losst, out_tensor_with_E, input_tensor, targets, output_tensor_cls, target_key, target_tensor, loss_cls, loss_energy
        gc.collect()
    ...
    return average_loss, average_cls_loss, average_energy_loss
    ...
    with torch.no_grad():
        for batch_idx, (input_coords, input_feats, target_coords, target_feats, masks, target_l) in enumerate(val_loader):
            ...
            if val_train:
                ...
                print(f"Validation: Iter: {batch_idx}, Loss: {losst.item():.3f}, Loss_cls: {loss_cls.item():.3f}, Loss_energy: {loss_energy.item():.3f}")
...
def test(model, test_loader, val_train=False, val_test=False, optimizer=None):
    ...
    with torch.no_grad():
        for batch_idx, (input_coords, input_feats, target_coords, target_feats, masks, target_l) in enumerate(test_loader):
            ...
            #    string_id = "target"
            #)
            out_tensor_with_E, targets, output_tensor_cls, losst, loss_cls, loss_energy = model(input_tensor, 0, 0, val_train=val_train, val_test=val_test, test=True)
            #output_tensor.F[:] = 1
            #print(targets)
            print(len(input_tensor), "->", len(out_tensor_with_E), "/", len(target_coords), 'target_coords_list: ', len(target_l))
            ...
            #output_tensor_save = ME.SparseTensor(features=output_tensor_energy.F, coordinates=output_tensor.C, device=device)
            output_tensor_save = out_tensor_with_E
            ...
            t = time() - s
            #print(f"Iter: {batch_idx}, Loss: {losst.item():.3f}, Data Loading Time: {d:.3f}, Total Time: {t:.3f}")
            #print(f"Test: Iter: {batch_idx}, Loss: {losst.item():.3f}")
            #print('HEEERRREEEE', len(tg_tensor_comp.F), len(tg_tensor_comp.C))
            input_coords_l.append(input_tensor.C.detach().cpu().numpy())
            target_coords_l.append(target_coords.detach().cpu().numpy())
            predictions_coords.append(output_tensor_save.C.detach().cpu().numpy())
            input_feats_l.append(input_tensor.F.detach().cpu().numpy())
            #print(target_feats)
            target_energy_l.append(target_feats.detach().cpu().numpy())
            predictions_energy.append(output_tensor_save.F.detach().cpu().numpy())
            #EPOCH_LOSS.append(losst.item()))

            #total_loss += losst.item()

           
            del losst, out_tensor_with_E, input_tensor, targets, output_tensor_cls, loss_cls, loss_energy
            gc.collect()
            torch.cuda.empty_cache()
            count += 1
    ...
    return predictions_coords, predictions_energy, input_coords_l, input_feats_l, target_coords_l, target_energy_l


# Early stopping parameters
patience = 10*5  # Number of epochs to wait for improvement
best_val_loss = float('inf')
epochs_no_improve = 0
warm_up_epochs = 50
            
for epoch in range(start_epoch, num_epochs):
    train_loss, train_cls_loss, train_energy_loss = train(model, train_DL, optimizer, epoch)
    val_loss, val_cls_loss, val_energy_loss = validate(model, val_DL, val_train=True, val_test=False)
    #val_train_loss = validate(model, val_DL, val_train=True, val_test=False)
    #val_test_loss = validate(model, val_DL, val_train=False, val_test=True)
    predictions_coords, predictions_energy, input_coords, input_feats, target_coords, target_energy = test(model, test_DL, val_train=False, val_test=False)
    base_dir = '/path_to_somewhere/base_directory/'
    ...
    # Log losses to W&B
    wandb.log({"epoch": epoch, "train_loss": train_loss, "train_cls_loss": train_cls_loss, "train_energy_loss": train_energy_loss, "val_loss": val_loss, "val_cls_loss": val_cls_loss, "val_energy_loss": val_energy_loss})
    ...
    if val_loss < best_val_loss:
        ...
        save_checkpoint(model, optimizer, epoch, path=f'/path_to_somewhere/best_checkpoint.pth')
    ...
    if epoch % 1 == 0 or epoch == num_epochs - 1:
        save_checkpoint(model, optimizer, epoch, path=f'/path_to_somewhere/checkpoint_epoch_{epoch}.pth')
    '''
    if epoch/num_epochs >= 9/10 or epoch % 1 == 0:
        predictions_coords, predictions_energy, input_coords, input_feats, target_coords, target_energy = test(model, test_DL, val_train=False, val_test=False)
    ...
    if epochs_no_improve >= patience and epoch >= warm_up_epochs:
        predictions_coords, predictions_energy, input_coords, input_feats, target_coords, target_energy = test(model, test_DL)
        base_dir = '/path_to_somewhere/base_directory/'
        save_predictions(base_dir, predictions_coords, predictions_energy, input_coords, input_feats, target_coords, target_energy, epoch)
