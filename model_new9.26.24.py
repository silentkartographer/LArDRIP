class CompletionNet_CutOnRawE_300(nn.Module):
    ...
    def sinkhorn_loss(self, track1_full, track2_full, xmin=114, xmax=128, ymin=140, ymax=154, epsilon=0.001, max_iters=350, tol=1e-8):
        ...
        for track_id in track_ids:
            ...
            #C /= C.max()
            C = C / (C.max() + 1e-9) # Add small epsilon to prevent division by zero
    ...
    def masked_loss_energy_real_chunk(self, dec_sx_cls, target, target_coords, crit_energy, device, xmin=138, xmax=152, ymin=140, ymax=154, use_de_dx_loss=False, calculate_in_inactive=True):
        ...
        chunk_size = 1  # trying to avoid OOM
        ...
        if use_de_dx_loss and pred_coords_masked.size(0) != 0:
            ...
            for batch in batch_numbers:
                ...
                # We know that abs(dE/dx) <= 2.1 MeV/cm, so
